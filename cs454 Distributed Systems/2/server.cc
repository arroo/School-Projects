...
void titlecase(char *in, unsigned long size) { // converts a string to title case
    int s = 1;
...
int getacceptor() { // creates a file descriptor for a listening socket
    addrinfo hints,
             *addlist;
    ...
    for( addrinfo *temp = addlist; temp != NULL; temp = temp->ai_next) {
        ...
        cout << "SERVER_ADDRESS " // print out the address and port
             << name              // for the client to connect
             << "\nSERVER_PORT "
             << ntohs(tsock.sin_port)
             << endl;
        break;
    }
    if (!out) {
    ...
    if (listen(out, 5) == -1) { // try to listen to up to
                                // five connections at a time
    ...
    while(1) {
        ...
        for(int i = 0; i <= highestclient; i++) {
            if (FD_ISSET(i, &accepting)) {
                if (i == acceptor) { // new connection
                ...
                } else { // client connected & giving string
                    ...
                    cout << buf << endl;
                    titlecase(buf,messagelen);
                    for (total = 0; total < messagelen; total += communicated) {
                        communicated = send(i,buf+total,messagelen-total,0);
                        if (communicated <= 0) {
                            close(i);
                            FD_CLR(i,&all);
                            break;
